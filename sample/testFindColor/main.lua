...
print("全屏查找刚才显示的图片")
local x,y = Display:findFeatureByShiftColor(0,0,maxX-1,maxY-1,color,feature,shiftColor,0,0)
print("找到的坐标是多少",x,y)


if x >-1 then
    print("找到颜色 0x333333 的计数",Display:getColorCountByShiftColor(x,y,x+50,y+50,0x333333,shiftColor))
    ...
    print("打印比色特征结果",Display:isFeatureByShiftColor(newFeature,shiftColor,0))
end
floatView:conceal()
print("等待一段时间确保悬浮窗口已经隐藏了")
Thread.sleep(1000)
print("如果上面找到正常的话，这里依旧可以再次找到")
x,y = Display:findFeatureByShiftColor(0,0,maxX-1,maxY-1,color,feature,shiftColor,0,0)
print("找到的坐标是多少",x,y)
